...
* Models
  ...
  1. A random forest classifier, with the following paramters:
     ...
     These hyperparameter aren't meant to create an outperforming classifier, but
  ...
  3. A Convolutional Neural Network, built as follow:
     ...
     The optimizing function is a Nadam procedure with learning rate equals to
     $0.002$.
...
* Dataset source
  ...
  The other dataset is directly the HG38 human genome assembly, retrived thanks
...
* Data correlation and distribution
  ...
  $0.873\%$ of the samples leads to an active region. This situation doens't
  ...
  while it is labeled as inactive with values lower than 1 for APvsIP task, while
...
+ The Feed-Forward Neural Network is the most powerful model and it seems quite
  robust. Wilcoxon test confims that it outperforms all the other models in
  both tasks.

+ All the models perform better in the promoters task instead of the enhancers,
  meaning that predicting the activation status of enanchers is harder than
  predicting the activation status of promoters.

* Further improvements

  There are several improvements that can been done to further investigate the
...
+ trying differnt values of TPM thresholds;
